...
def create_app_icon(custom_image_path=None):
    ...
    if custom_image_path and os.path.exists(custom_image_path):
        logger.info("IconCreator", f"Using custom image: {custom_image_path}")
        try:
            ...
            logger.error("IconCreator", f"Error processing image: {str(e)}")
            return False
    else:
        logger.info("IconCreator", "No custom image provided or file not found. Creating default icon...")
    ...
    try:
        ...
        logger.info("IconCreator", "Icon files created successfully!")
        logger.info("IconCreator", f"ICO file: {os.path.abspath('assets/icon.ico')}")
        logger.info("IconCreator", f"PNG file: {os.path.abspath('assets/icon.png')}")
        logger.info("IconCreator", f"Small PNG file: {os.path.abspath('assets/icon_small.png')}")
        return True
    except Exception as e:
        logger.error("IconCreator", f"Error saving icon files: {str(e)}")
        return False
...
def select_and_create_icon():
    ...
    if file_path:
        ...
        logger.info("IconCreator", "No file selected. Creating default icon...")
        create_app_icon()
